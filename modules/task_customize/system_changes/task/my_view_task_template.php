...
$CI = &get_instance();
$CI->load->helper('task_customize');
$task_custom_field = task_custom_field($task->id);
$task_custom_field_value = isset($task_custom_field['value']) ? $task_custom_field['value'] : '';
$task_custom_field_name = isset($task_custom_field['name']) ? $task_custom_field['name'] : '';
$task_custom_field_id = isset($task_custom_field['id']) ? $task_custom_field['id'] : '';
?>
...
<div class="modal-body">
        ...
        <div class="col-md-8 task-single-col-left">
            <div class="tw-mb-4">
                ...
                <?php if (! empty($task->rel_id)) {
                    ...
                    $task_rel_value = my_get_relation_values($task_rel_data, $task->rel_type);
    ...
    <div class="col-md-4 task-single-col-right">
        <div class="pull-right mbot10 task-single-menu task-menu-options">
            ...
            <input type="hidden" id="copyURL" value="<?= admin_url('tasks/view/' . $task->id); ?>">
            <button class="copy-btn" onclick="copyToClipboard(this)">
                <i class="fa fa-clipboard"></i> <!-- Default Copy Icon -->
            </button>
        ...
        <div class="task-info task-single-inline-wrap task-info-start-date">
            <h5 class="tw-inline-flex tw-items-center tw-space-x-1.5">
                ...
                <?php if (staff_can('edit', 'tasks') && $task->status != 5) { ?>
                    <input name="work_planed" tabindex="-1"
                        value="<?= e(_d($task_custom_field_value)); ?>"
                        id="task-single-work_planned"
                        class="task-info-inline-input-edit datepicker pointer tw-text-neutral-800" data-task_id="<?php echo $task->id; ?>"
                        data-field_id="<?php echo $task_custom_field_id; ?>">
                <?php } else { ?>
                    <span class="tw-text-neutral-800">
                        <?= e(_d($task_custom_field_value)); ?>
                    </span>
                <?php } ?>
        ...
        <div class="task-info">
            <h5 class="task-info-custom-field tw-inline-flex tw-items-center tw-space-x-1.5 task-info-custom-field-cam_id">
                ...
                <?= _l('cam_id'); ?>:
                <span class="tw-text-neutral-800">
                    <?= isset($task->cam_id) && $task->cam_id != '' ? get_staff_full_name($task->cam_id) : 'N/A'; ?>
                </span>
            </h5>
        </div>
        <div class="task-info">
            <h5 class="task-info-custom-field tw-inline-flex tw-items-center tw-space-x-1.5 task-info-custom-field-optimizer_id">
                ...
                <?= _l('optimizer_id'); ?>:
                <span class="tw-text-neutral-800">
                    <?= isset($task->optimizer_id) && $task->optimizer_id != '' ? get_staff_full_name($task->optimizer_id) : 'N/A'; ?>
                </span>
            </h5>

        </div>
        <div class="task-info">
            <h5 class="task-info-custom-field tw-inline-flex tw-items-center tw-space-x-1.5 task-info-custom-field-organic_social_id">
                <i class="fa-regular fa-circle fa-fw fa-lg task-info-icon"></i>
                <?= _l('organic_social_id'); ?>:
                <span class="tw-text-neutral-800">
                    <?= isset($task->organic_social_id) && $task->organic_social_id != '' ? get_staff_full_name($task->organic_social_id) : 'N/A'; ?>
                </span>
            </h5>
        ...
        <div class="task-info">
            <h5 class="task-info-custom-field tw-inline-flex tw-items-center tw-space-x-1.5 task-info-custom-field-seo_lead_id">
                ...
                <?= _l('seo_lead_id'); ?>:
                <span class="tw-text-neutral-800">
                    <?= isset($task->seo_lead_id) && $task->seo_lead_id != '' ? get_staff_full_name($task->seo_lead_id) : 'N/A'; ?>
                </span>
            </h5>

        </div>
        <div class="task-info">
            <h5 class="task-info-custom-field tw-inline-flex tw-items-center tw-space-x-1.5 task-info-custom-field-sale_rep_id">
                <i class="fa-regular fa-circle fa-fw fa-lg task-info-icon"></i>
                <?= _l('sale_rep_id'); ?>:
                <span class="tw-text-neutral-800">
...
<script>
    ...
    function copyToClipboard(button) {
        ...
        tempInput.value = document.getElementById("copyURL").value;
        ...
        icon.classList.add("fa-check", "text-green");

        // Reset to clipboard icon after 2 seconds
        setTimeout(function() {
            icon.classList.remove("fa-check", "text-green");
    ...
    $(document).ready(function() {
        ...
        $('#knifeSwitchLabel').on('click', function() {
            const $checkbox = $('#myonoffswitch');
            ...
            if ($checkbox.prop('checked')) {
                ...
                $('svg #_x32_').attr('fill', 'crimson');
                $('svg #_x32_').attr('stroke', 'crimson');
            } else {
                $(this).css({
                    color: '#ccc',
                    transform: 'none'
        ...
        $('.onoffswitch-checkbox').on('change', function() {
            $.ajax({
                ...
                type: "POST",
                data: {
                    is_poked: $(this).prop('checked') ? 1 : 0,
