...
<script>
    ...
    $(function() {
        ...
        var $field = $('select[name="custom_fields[contracts][28]"]');

        // ✅ Set default to "Active" only if nothing is selected
        if ($field.length && !$field.val()) {
            $field.val('Active').trigger('change');
        }

        var contractServerParams = {
            contract_status: "[name='custom_fields[contracts][28]']",
        };

        initDataTable(
            '.table-contracts-single-client',
            admin_url + 'contracts/table/' + customer_id,
            undefined,
            undefined,
            contractServerParams,
            [6, 'desc']
        );
