    ...
    <script>
        ...
        $(function() {
            ...
            _rel_type.on('change', function() {
                ...
                //check type is project , customer , contract
                if (['project', 'customer', 'contract'].includes($(this).val())) {
            ...
            $('body').on('change', '#rel_id', function() {
                if ($(this).val() != '') {
                    ...
                    //get data according _rel_type and set in selct dropdown in custome_select
                    if (_rel_type.val() == 'project') {
                        //get projects data
                        $.get(admin_url + 'task_customize/get_project_details/' + $(this).val(), function(data) {
                            if (data) {
                    ...
                    if (_rel_type.val() == 'customer') {
                        $.get(admin_url + 'task_customize/get_customer_details/' + $(this).val(), function(data) {
                            if (data) {
                                var data = JSON.parse(data);
                                if (!$.isEmptyObject(data)) {
                                    $('#cam_id').val(data.cam_id);
                    ...
                    if (_rel_type.val() == 'contract') {
                        $.get(admin_url + 'task_customize/get_contract_details/' + $(this).val(), function(data) {
                            var data = JSON.parse(data);
                            if (data) {
                                $('#cam_id').val(data.cam_id);
                                $('#cam_id').selectpicker('refresh');
